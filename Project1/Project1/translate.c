...
Tr_accessList Tr_formals(Tr_level level) {
    F_accessList formals = F_formals(level->frame)->tail; 
...
Tr_exp Tr_simpleVar(Tr_access access, Tr_level level) {
	if (access->level == level) {
		...
		T_exp fp = makeStaticLink(level, access->level->parent);
...
Tr_exp Tr_stringExp(string s) {
    Temp_label label = Temp_newlabel();
    F_frag fragment = F_StringFrag(label, s);
    addFrag(fragment); 
...
Tr_exp Tr_stringCompare(A_oper op, Tr_exp tr_left, Tr_exp tr_right) {
    return Tr_opExp(op, Tr_Ex(F_externalCall("wrap_strcmp", T_ExpList(unEx(tr_left), T_ExpList(unEx(tr_right), NULL)))),
            Tr_Ex(T_Const(0)));
}
...
Tr_exp Tr_seqExp(Tr_expList exps, bool hasRV) {
    ...
    if (!hasRV) {
        ...
        return Tr_Nx(seq);
		*/
		T_stm seq = T_Seq( unNx(exps->tail->head),unNx(exps->head));
		T_stm cur = seq;
		exps = exps->tail->tail;
		for (; exps; exps = exps->tail) {

			cur->u.SEQ.left = T_Seq(unNx(exps->head),cur->u.SEQ.left);
		}
		return Tr_Nx(seq);
    }
...
{
	T_stm stm;
	stm = T_Move(T_Temp(F_RV()), unEx(bodylist->head));
	bodylist = bodylist->tail;
	while (bodylist)
	{
		stm = T_Seq(T_Move(T_Temp(F_RV()),unEx(bodylist->head)), stm);
		bodylist = bodylist->tail;
	}
	return Tr_Nx(stm);
}
