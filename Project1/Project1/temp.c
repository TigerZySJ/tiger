...
Temp_tempList Temp_TempList_Copy(Temp_tempList t) {
	Temp_tempList list_cpy = (Temp_tempList)checked_malloc(sizeof(*list_cpy));
	for (; t; t = t->tail) {
		Temp_temp tmp_cpy = t->head;
		if (list_cpy->head == NULL)
			list_cpy->head = tmp_cpy;
		else
			list_cpy->tail = Temp_TempList(tmp_cpy,NULL);
	}
	return list_cpy;
}
...
Temp_tempList Temp_remove(Temp_temp one, Temp_tempList list) {
	Temp_tempList now;
