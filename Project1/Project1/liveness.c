...
static struct Live_graph interferenceGraph(G_nodeList nodelist,G_table out) {
	//初始化图
	G_graph g = G_Graph();
...
struct Live_graph Live_liveness(G_graph flow) {
	...
	struct Live_graph graph = interferenceGraph(G_nodes(flow), out);
}
Live_moveList Live_moveList_Copy(Live_moveList t1) {
	Live_moveList t2 = (Live_moveList)checked_malloc(sizeof(*t1));
	for (; t1; t1 = t1->tail) {
		t2 = Live_MoveList(t1->src, t1->dst, t2);
	}
	return t2;
}
Live_moveList intersection_Live_moveList(Live_moveList t1, Live_moveList t2) {
	...
	if (!t2)
		return NULL;
	Live_moveList t3 = NULL;

	for (; t2; t2 = t2->tail) {
		bool flag = 0;
		for (t1 = t3; t1; t1 = t1->tail)
			if (t1->dst == t2->dst&&t1->src == t2->src) {
				flag = 1;
				break;
			}
		//如果在t1中，加入t3
		if (flag)t3 = Live_MoveList(t1->src, t1->dst, t3);
	}
	return t3;
}
...
Live_moveList union_Live_moveList(Live_moveList t1, Live_moveList t2) {
	...
	if (!t2)
		return t1;
	Live_moveList t3 = Live_moveList_Copy(t1);
