...
G_nodeList G_nodeList_Copy(G_nodeList t1) {
	G_nodeList t2 = (G_nodeList)checked_malloc(sizeof(*t1));
	for (; t1; t1 = t1->tail) {
		G_node tmp_cpy = t1->head;
		t2= G_NodeList(tmp_cpy, t2);
	}
	return t2;
}
G_nodeList union_G_nodeList(G_nodeList t1, G_nodeList t2) {
	if (!t1)
		return t2;
	if (!t2)
		return t1;
	G_nodeList t3 = G_nodeList_Copy(t1);

	for (; t2; t2 = t2->tail) {
		bool flag = 0;
		for (t1 = t3; t1; t1 = t1->tail)
			if (t1->head == t2->head)
				flag = 1;
		//如果不在t1中，加入t1
		if (!flag)t3 = G_NodeList(t2->head, t3);
	}
	return t3;
}
...
//求补集
G_nodeList except_G_nodeList(G_nodeList t1, G_nodeList t2) {
	if (!t1)
		return NULL;
	if (!t2)
		return t1;
	...
	for (; t1; t1 = t1->tail) {
		bool flag = 0;
		for (t2 = t4; t2; t2 = t2->tail)
			if (t1->head == t2->head)
				flag = 1;
		//t1元素如果不在t2中，加入t3
		if (!flag)t3 = G_NodeList(t1->head, t3);
	}
	return t3;
}

G_nodeList intersection_G_nodeList(G_nodeList t1, G_nodeList t2) {
	if (!t1)
		return NULL;
	if (!t2)
		return NULL;
